    ...
    <div class="sidebar">
      ...
      <!-- 情绪分析 -->
       <div class="emotion-garden">
        ...
        <div class="emotion-info" @click="tiaozhuang">
        ...
        <div class="warm-tips">
          ...
          <div class="emotion-intensity">
            <span class="intensity-dots">
              <span v-for="dot in 3" :key="dot" class="dot" :class="{'active':getIntersityClass(currentEmotion.emotionScore)>=dot}"></span>
            </span>
            <span class="intensity-text">
              {{ getRickText(currentEmotion.riskLevel) }}
            </span>
          </div>
          <!-- 温暖建议卡片 -->
          ...
          <!-- 治愈行动 -->
           ...
           <!-- 风险提示 -->
            <div v-if="currentEmotion.riskLevel>1 && currentEmotion.isNegative" class="risk-notice">
    ...
    <div class="chat-main">
      ...
      <!-- 输入区 -->
       <div class="chat-input">
        ...
        <el-button :disabled="!userMessage.trim() || userMessage.length>500" class="send-btn" @click="sendMessage">
          <el-icon>
            <Promotion/>
          </el-icon>
        </el-button>
...
const createNewFrontendSession=()=>{
  ...
  messages.value = []
  currentEmotion.value=startEmotion
}
...
const sendMessage=()=>{
  ...
  if(isAiTyping.value){
    ElMessage.error('AI助手正在输出中，请稍后再发送')
    return
  }
  //可以发送，发送完清空对话框内容
...
const startAIResponse=(sessionId,userMessage)=>{
  ...
  fetchEventSource(`/api/psychological-chat/stream`,{
    ...
    onerror:(error)=>{
      handleError(error||'AI助手失败，请稍后再重试')
      throw error
    },
    onclose:()=>{
      loadSessionEmotion(currentSession.value.sessionId)
...
const handleError=(error)=>{
  const aiMessage = messages.value[messages.value.length-1]
  if(aiMessage){
    aiMessage.content='AI助手失败，请稍后再重试'
  }
  isAiTyping.value=false
  ElMessage.error('AI助手失败，请稍后再重试')
  }
